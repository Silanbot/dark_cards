...
async function sendJoinEvent() {
    let { $players } = await useConnection(room.value.id, user.value)
    $players = $players.filter(p => parseInt(p.id) !== parseInt(user.value.id))
    ...
    for (const cardHand of document.querySelectorAll('img.my-card')) {
        ...
        if (lt(card.dataset.card, cardHand.dataset.card)) {
            ...
            document.querySelector('#cards').insertBefore(card, cardHand)
             hand.value.insertBefore(card, cardHand)
        }
    }
...
onMounted(async () => {
    ...
    $websocket.addListener({ event: 'player_take_card', handler: eventPlayerTakeCard })
    $websocket.addListener({ event: 'player_take_table', handler: eventPlayerTakeTable })
    $websocket.addListener({ event: 'user_left_room', handler: eventPlayerLeft })
    $websocket.addListener({ event: 'discard_card', handler: eventDiscardCard })
    $websocket.addListener({ event: 'revert_card', handler: eventRevertCard })
    $websocket.addListener({ event: 'beats', handler: eventBeats })
    $websocket.addListener({ event: 'user_win', handler: eventPlayerWin })
    ...
    <section class="inter section section-inter">
                ...
                <div class="game__players">
                    <div class="game__players__player" :key="i" v-for="(player, i) of players" :class="(() => {
                        ...
                        <div class="game__players__player__photo" :data-player="player.id">
                            ...
                            <img class="game__players__player__photo__img" :src="player.avatar" alt=""/>
                ...
                <div class="game__cart">
                    <div class="game__cart__cold">
                        ...
                        <div class="game__cart__cold__carts">
                            ...
                            <img class="game__cart__cold__carts__second" ref="trump" src="" alt=""/>
                        </div>
                    </div>
                </div>
            </section>
        ...
        <footer class="footer" style="z-index: 100">
            <div class="footer__inner footer__inner_play">
                ...
                <div class="footer__button chat" v-if="!isAttackerPlayer" @click="() => {}">
                ...
                <div class="footer__btns" v-if="isAttackerPlayer">
