...
<!--                    <div class="footer__button"                                v-else-if="myTurn">Ваш ход</div>-->
                    <div class="footer__button" @click="() => takeFromTable()" v-else :style="{ visibility: (!myTurn && gameCells.filter(c => c.length).length > 0 && gameCells.filter(c => c.length).every(c => c.length % 2 !== 0)) ? 'hidden' : undefined }">Взять</div>
                </template>
