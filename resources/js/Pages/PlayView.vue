...
<!--                    <div class="footer__button"                                v-else-if="myTurn">Ваш ход</div>-->
                    <div class="footer__button" @click="() => takeFromTable()" v-else :style="{ visibility: (myTurn  && gameCells.every(c => !c.length)) || (gameCells.filter(c => c.length).length === 0) ? 'hidden' : undefined }">Взять</div>
                </template>
